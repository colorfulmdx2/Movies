...
export const MovieElement = (props: PropsType) => {
    ...
    const addMovieToFavorite1 = (movie: any)=> {

        dispatch(addMovieToFavorite(movie))
    }

    const removeMovieFromFavorite1 = (movie: any)=> {
            ...
            <CardActions className={classes.buttons}>
                {
                    user !== null
                        ? favoritesMoviesData[props.id]
                            ? <Button onClick={() => { removeMovieFromFavorite1(props.movie) }} size="medium" color="secondary" variant={"contained"}>
                                Remove from Favorites
                            </Button>
                            : <Button onClick={() => { addMovieToFavorite1(props.movie) }} size="medium" color="secondary" variant={"contained"}>
                                Add to Favorites
                            </Button>
